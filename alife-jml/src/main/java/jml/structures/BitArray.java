...
public class BitArray implements Cloneable {
 ...
 private int[] data = null;
  ...
  public BitArray(int n, boolean randomly) {
    ...
    data = new int[m];
    if (randomly) {
      UniformNumberGenerator g = new UniformNumberGenerator(IntUtil.HIGHEST_BIT >>> 1);
      for (int i = 0; i < m; i++) {
        if (g.newBoolean()) { data[i] = g.newInt(); 
        } else { data[i] = -g.newInt(); }
      }
    } else {
      for (int i = 0; i < m; i++) {
        data[i] = 0;
  ...
  public static BitArray random(int limit) {
    ...
    UniformNumberGenerator g = new UniformNumberGenerator(limit);
    int[] d = new int[]{g.newInt() };
    System.out.println(d[0]);
    BitArray a = new BitArray(size, true);
    a.data = d;
    return a;
  }
