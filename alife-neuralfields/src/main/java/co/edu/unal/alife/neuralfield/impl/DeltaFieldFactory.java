...
import co.edu.unal.alife.evolution.param.S1RepresentationParameters;
...
public class DeltaFieldFactory {
	...
	public AbstractDeltaField<Double> buildLayeredS1Field(
		...
		AbstractNonDifferentialEquation actionEquation = new S1ActionEquation(
		...
		for (int i = 0; i < noGoals; i++) {
			List<AbstractKernelFunction> inputRow = null;
		...
		for (int i = 0; i < noGoals; i++) {
			ArrayList<AbstractKernelFunction> goalKernelRow = new ArrayList<AbstractKernelFunction>();
		...
		List<AbstractKernelFunction> actionRow = null;
		kernelMatrix.add(actionRow);
		List<AbstractKernelFunction> systemRow = null;
