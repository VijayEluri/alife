...
public class S1InputEquationForPendulum extends NonDifferentialEquation {
	 ...
	 * @param halfSize
	 */
	public S1InputEquationForPendulum(DeltaPopulation<Double> pendulum, List<Double> ks) {
		super();
		this.pendulum = pendulum;
		this.ks = ks;
		if(ks.size()!=pendulum.getSize()) {
			throw new InputMismatchException("The number of Ks and of SystemPopulation elements must be equal");
		}
	}
...
//		System.out.println("OMEGA:" + omega + "->" + boundedValue2 + ":" + eqPosition2);
		...
		double boundedValue3 = S1TopologyUtility.angleToPosition(S1TopologyUtility.reverseStereoProjection(x),points);
		double eqPosition3 = Math.round(boundedValue3);
		
		double v = pendulum.getElementValue(PendulumEquation.STATE_V);
		double boundedValue4 = S1TopologyUtility.angleToPosition(S1TopologyUtility.reverseStereoProjection(v),points);
