...
public class RungeKutta4thSolver extends AbstractSolver<Double> {	
	...
	public RungeKutta4thSolver(List<Double> x, double h, double t,
			Derivable<Double> f, boolean isTraceEnabled, double tracePeriod) {
		super(x, h, t, f, isTraceEnabled, tracePeriod);
	}
	...
	public List<Double> step() {
		...
		for (int i = 0; i < x.size(); i++) {
			xAtK2.set(i, x.get(i) + h / 2 * k1dh.get(i));
		}
		k2dh = f.getDeltas(xAtK2, t + h / 2);
		for (int i = 0; i < x.size(); i++) {
			xAtK3.set(i, x.get(i) + h / 2 * k2dh.get(i));
		}
		k3dh = f.getDeltas(xAtK3, t + h / 2);
		for (int i = 0; i < x.size(); i++) {
			xAtK4.set(i, x.get(i) + h * k3dh.get(i));
		}
